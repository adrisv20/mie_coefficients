#Este código representa las eficiencias de scattering y extinción frente a la longitud
#de onda lambda con un core-shell quiral y con absorción (eh es complejo).
...
# Evaluar Qsca y Qext para cada valor de lambda
...
fig.suptitle("$Q_{s}$ y $Q_{ext}$ vs $\\lambda[nm]$ ($\\sigma=1$, $\\kappa=1$ y núcleo con absorción)", fontsize=14)
# Gráfico 1: Qs vs kappa
axs[0].plot(lamb_nm, qsca_vals, marker='o', color='mediumseagreen', linestyle='-', linewidth=1)
axs[0].set_xlabel("$\\lambda[nm]$")
axs[0].set_ylabel("$Q_{s}$")
axs[0].set_ylim([-0.3, 6.5])
axs[0].grid(True)
